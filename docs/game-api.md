...
| `getPlayer(id)` | `GET /api/v1/Player/{id}` | one player's full detail, including `intelligenceReport` when the caller has vision — **breaking change, live 2026-09-21**: the bare `playerLevel` int is gone, replaced by `playerLevelDetails: {level, progressPercent, xpEarnedInLevel, xpRequiredForNextLevel, xpRemainingToNextLevel, totalXp}`; `mapPlayerDetailToSyncPayload` reads `playerLevelDetails.level` (the other five sub-fields aren't captured yet — no column for them) |
