...
| `getPlayers()` | `GET /api/v1/Player` | all active players (no filter): `[{id, allianceId, isActivePlayer, name, allianceTag, joinedAt, playerLevel, playsFromCountryCode, pointsScored, rank}]` — **breaking change, live 2026-09-21**: `playerLevel` is now a `decimal` (e.g. `2.65` = Level 2, 65% to Level 3), not an `int`; `mapPlayersToSyncPayload` floors it |
| `getPlayer(id)` | `GET /api/v1/Player/{id}` | one player's full detail, including `intelligenceReport` when the caller has vision — **breaking change, live 2026-09-21**: the bare `playerLevel` int is gone, replaced by `playerLevelDetails: {level, progressPercent, xpEarnedInLevel, xpRequiredForNextLevel, xpRemainingToNextLevel, totalXp}`; `mapPlayerDetailToSyncPayload` carries the whole object through to `players.level`/`level_progress_percent`/`xp_earned_in_level`/`xp_required_for_next_level`/`xp_remaining_to_next_level`/`total_xp` — only populated once a player has had at least one detail scan, not from the list endpoint |
