// Regression coverage for /sync/highest-population-snapshot (2026-09-18, Various Changes) --
// mirrors sync-best-planets-coverage.test.js: public and aggregate-only, "how many of the
// current snapshot are friendly-owned", resolved via our own synced ownership, never the
// ranking page's own owner text.
//
// Run with: node src/routes/sync-highest-population-coverage.test.js
...
const tmpDb = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'awt-sync-highest-population-test-')), 'test.db');
...
console.log('sync-highest-population-coverage.test.js');
...
(async () => {
    ...
    try {
        ...
        db.prepare(`INSERT INTO planets (game_planet_id, system_id, planet_index, owner_id, population) VALUES
            (97001, 970, 1, 811, 10), (97101, 971, 1, 812, 9)`).run();

        console.log('\n── An invalid payload is rejected ' + '─'.repeat(41));
        const badRes = await request(server, 'POST', '/hub-api/sync/highest-population-snapshot', {});
        ok('missing rows is a 400', badRes.status === 400, badRes.body);

        console.log('\n── First snapshot: coverage recomputed and stored ' + '─'.repeat(24));
        const firstRes = await request(server, 'POST', '/hub-api/sync/highest-population-snapshot', {
        ...
        const snapshotRows = db.prepare(`SELECT COUNT(*) as n FROM highest_population_snapshot`).get().n;
        ok('both rows are stored', snapshotRows === 2, snapshotRows);

        console.log('\n── A re-sync with unchanged coverage does not throw ' + '─'.repeat(22));
        const secondRes = await request(server, 'POST', '/hub-api/sync/highest-population-snapshot', {
            rows: [{ rank: 1, game_planet_id: 97001 }, { rank: 2, game_planet_id: 97101 }],
        });
        ...
        db.prepare(`INSERT INTO planets (game_planet_id, system_id, planet_index, owner_id, population) VALUES (97201, 972, 1, 811, 8)`).run();
        const thirdRes = await request(server, 'POST', '/hub-api/sync/highest-population-snapshot', {
        ...
        const snapshotAfterThird = db.prepare(`SELECT COUNT(*) as n FROM highest_population_snapshot`).get().n;
        ok('the snapshot now has all three rows', snapshotAfterThird === 3, snapshotAfterThird);

        const lastAnnounced = db.prepare(`SELECT value FROM app_settings WHERE key = 'highest_population_friendly_count_last_announced'`).get();
