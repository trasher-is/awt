...
console.log('\n── Per-system milestone channel ' + '─'.repeat(45));
{
    ...
    const siege = { planet_index: 3, type: 'SIEGE_STARTED', owner: '[RAID] Holder', owner_alliance_tag: 'RAID', attacker_name: 'Raider1' };
    ...
    // reaches this builder is already known-hostile and needs no allegiance field.
    ok('a siege event with no attacker name still renders, just without naming anyone',
        buildSystemMilestoneLines([{ planet_index: 12, type: 'SIEGE_STARTED', owner: '[RAID] Holder', owner_alliance_tag: 'RAID' }], friendly)
            .some(l => l.includes('Planet 12') && l.includes('under siege!') && !l.includes(' by ')));
